...
- [ ] No accidental orphan `.mdx` files: every managed content page appears in `docs.json` navigation, and every nav entry resolves to a file. A site root page may be deliberately hidden because `/` remains directly accessible, but hidden pages are excluded from search and assistant indexing by default; record that choice in the report. A homepage may also be registered as a dedicated top-level page/tab. Do not place it in an existing sidebar group unless the source shows that row; see Gate 3 *Site root not duplicated as sidebar entry*. On failure → `/style-docs` "Restructuring a large, flat navigation".
...
- [ ] **Website-first landing parity** (`index.mdx`): identify and screenshot the customer's canonical marketing/product website in addition to the source docs. The landing page must reuse its recognizable visual system—logo composition, typography, colors, backgrounds/textures, button and card language, media style, and density—while preserving docs-first IA. Hide the logo in the comparison: if the first fold could belong to any company, this check fails. A generic gradient hero or generic icon-card template is not acceptable when source-backed styling or media exists.
...
- [ ] **Navigation surface choice is intentional.** Review root groups, tabs, products, and versions against the source and intended user journeys. An empty header is valid when a continuous sidebar is clearer; tabs and switchers are valid when they solve a concrete discovery or context-switching problem. Do not fail a layout from section count alone.
- [ ] **Top horizontal-bar single-row parity.** Open the source navbar and count the items it paints in its inline horizontal row, left-to-right. When preserving that row, every item belongs in `navigation.tabs` in the same order — internal pages as page references, external destinations as `tabs` entries with `href`. Do not split one visual row across `navigation.tabs`, `navbar.links`, and `navigation.global.anchors`. A product or version switcher is an acceptable documented redesign only when it solves a real context-switching problem, never merely because the row exceeds a fixed count. See `style-docs/SKILL.md` → *Top horizontal bar — one row, one config surface*.
...
- [ ] **Top-of-page tab toggle is justified.** Mintlify `tabs` render visible header navigation. Check source parity and the intended journey:
  - If the source shows a tab-button row, tabs are usually the closest mapping unless a documented redesign is clearer.
  - If the source has no tab row, do not add one automatically; use tabs only when separate sidebars and persistent header access solve a concrete problem.
  - Root-level sibling `groups` remain valid for any site where a continuous sidebar is clearer.
  
  Fetch the source page being mirrored and record any deliberate header adaptation. See `style-docs/SKILL.md` → *Choosing groups, tabs, products, and versions* and `style-docs/reference.md` → *Flattening inappropriate nested `tabs` to sibling `groups`*.
- [ ] **Site root not duplicated as a sidebar entry.** Open `docs.json` and confirm the site root slug (`"index"`, or whichever file Mintlify serves at `/`) is not inside an existing sidebar group unless the source deliberately renders a Home row there. A dedicated top-level Home page/tab is valid when it matches the source or provides required navigation-backed indexing. The classic defect is a Getting Started group containing both `Home` and `Overview`; remove `index` from that group or remove the duplicate sibling according to source parity. See `style-docs/reference.md` → *Site root `index.mdx` and the sidebar*.
...
- [ ] **Canonical website theme match (landing page).** Compare the local homepage against the customer's real marketing/product website, not only against the old docs. Verify font family/weight, heading scale, accent usage, border radius, button shape, section rhythm, card density, background pattern, and media treatment. The docs homepage should read as a natural extension of the website.
...
- [ ] **Asset integrity and cropping:** every logo/media URL returns the expected MIME type and real file contents; no HTML fallback is saved as an image. Check native dimensions and inspect representative video frames. No embedded words, controls, faces, or focal content are clipped by square/fixed-height wrappers or `object-cover`.
- [ ] **Card density:** large visual card areas contain source-backed product proof or a task-specific UI treatment. Empty patterned panels with a small generic icon fail when the customer's site provides richer visual material.
- [ ] Repeat in **dark mode**.
...
| 3 | Navigation is hard to scan or mixes unrelated journeys | Review root groups, tabs, products, and versions; choose the smallest surface that solves the observed problem, preserve the internal taxonomy, and do not infer the answer from section count. See `style-docs/SKILL.md` "Choosing groups, tabs, products, and versions" |
| 3 | Tabs or a switcher add chrome without improving navigation | Flatten to sibling `groups` per `style-docs/reference.md` "Flattening inappropriate nested tabs to sibling groups", or remove the unnecessary switcher |
...
| 3 | Per-page sidebar shows entries (group labels, wrapping-anchor labels, or pages) the source's same-URL sidebar doesn't show | Separate the leaking journeys using the least disruptive suitable surface—anchors, tabs, products, or versions—or keep root groups and refine their membership. Drop any wrapper `anchor` whose label isn't on the source. See `style-docs/reference.md` → "Per-dropdown sidebar anchors". Re-run `/preview-qa` from Gate 1 because the restructure can regress collapsibility and chrome |
